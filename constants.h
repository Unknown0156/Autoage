#pragma once

#include <QString>

const int TIMER_DELAY = 100; //таймер главного окна

const float TURN_PRECISION = 8.0f; //точность поворота
const float MOVE_TO_POINT_PRECISION = 2.0f; //точность движения к точке
const float MOVE_TO_MOB_PRECISION = 20.0f; //точность движения к таргету
const float FARM_RANGE=100.0f;//максимальное расстояние от начальной точки

const int NUMBER_OF_MOBS = 3+9+27+27*3;//колличество указателей на мобов
const int MOB_BASE = 969638352; //значение vtable моба


